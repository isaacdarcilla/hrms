  ...
  <div class="flex items-center">
    <div class="flex w-full bg-white shadow rounded-lg">
      <dropdown :auto-close="false" class="px-4 md:px-6 rounded-l border-r hover:bg-gray-100 focus:border-white focus:shadow-outline focus:z-10" placement="bottom-start">
        ...
        <div slot="dropdown" class="mt-2 px-4 py-6 w-screen shadow-xl bg-white rounded-lg" :style="{ maxWidth: `${maxWidth}px` }">
          <slot />
        </div>
      </dropdown>
      <input class="relative w-full px-6 py-3 rounded-r-lg focus:shadow-outline" autocomplete="off" type="text" name="search" placeholder="Search anything…" :value="value" @input="$emit('input', $event.target.value)">
    </div>
