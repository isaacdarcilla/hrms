            ...
            <div>
              <div class="mt-3 sm:mt-0 sm:ml-4 sm:text-left">
                <h3
                  ...
                  id="modal-headline"
                >
                  ✏️ Edit {{ employeeId.first_name }} Membership in Organizations
                </h3>
                <form @submit.prevent="save" class="w-full max-w-lg pr-4 pt-5">
                  <div class="flex flex-wrap -mx-3 mb-6">
                    <div class="w-full px-3">
                      <label class="form-label font-bold">Name of Membership <span class="text-red-600">*</span></label>
                      <input
                        autofocus="true"
                        class="form-input block w-full"
                        :placeholder="membership.memberships_name"
                        v-model="form.memberships_name"
                      />
                      <div
                        v-if="$page.errors.memberships_name !== null"
                        class="form-error"
                      >
                        {{ $page.errors.memberships_name }}
                      </div>
                    </div>
                  </div>
                </form>
...
export default {
  ...
  methods: {
    save() {
      this.$inertia.put(
        this.route("membership.update", this.membership.id),
        this.form,
        {
